...
// "Refactored to New Input System API for cross-platform support (Mobile, WebGL, Desktop)" || version 2.0.0-DIGGERS
...
public class FirstPersonController : MonoBehaviour
{
    ...
    // Cached input values for use in FixedUpdate
    private Vector2 cachedMoveInput;
    private bool cachedSprintPressed;
    private bool awaitingInitialPointerLock;

    /// <summary>
    /// Scale factor to normalize pointer delta values to approximate legacy Input.GetAxis behavior.
    /// </summary>
    private const float POINTER_DELTA_SCALE = 0.1f;
    ...
    void Start()
    {
        if(lockCursor)
        {
            ...
            // Browser security requires an explicit user gesture before pointer lock.
            awaitingInitialPointerLock = true;
    ...
    /// <summary>
    /// Reads the look input vector from the assigned look InputActionReference.
    /// </summary>
    ...
    /// <summary>
    /// Reads the move input vector from the assigned move InputActionReference.
    /// </summary>
    ...
    /// </summary>
    private bool WasActionReleasedThisFrame(InputActionReference actionRef)
    {
        return actionRef != null && actionRef.action != null && actionRef.action.WasReleasedThisFrame();
    }

    /// <summary>
    /// Returns true if the given action is currently held down.
    /// </summary>
    ...
    {
#if UNITY_WEBGL && !UNITY_EDITOR
        if (awaitingInitialPointerLock && lockCursor && Pointer.current != null && Pointer.current.press.wasPressedThisFrame)
        {
            Cursor.lockState = CursorLockMode.Locked;
            Cursor.visible = false;
        ...
        if (enableZoom)
        {
            // Changes isZoomed when action is triggered
            // Behavior for toggle zoom
            if(WasActionPressedThisFrame(zoomAction) && !holdToZoom && !isSprinting)
            {
                isZoomed = !isZoomed;
            }

            // Changes isZoomed when action is pressed/released
            // Behavior for hold to zoom
            if(holdToZoom && !isSprinting)
            {
                if(WasActionPressedThisFrame(zoomAction))
                {
                    isZoomed = true;
                }
                else if(WasActionReleasedThisFrame(zoomAction))
                {
                    isZoomed = false;
                }
            }
        ...
        if(enableJump && WasActionPressedThisFrame(jumpAction) && isGrounded)
        {
            Jump();
        }
        ...
        if (enableCrouch)
        {
            if(WasActionPressedThisFrame(crouchAction) && !holdToCrouch)
            {
                Crouch();
            }
            ...
            else if(WasActionReleasedThisFrame(crouchAction) && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
        ...
        // Cache input values for FixedUpdate consumption
        cachedMoveInput = ReadMoveInput();
        cachedSprintPressed = IsActionPressed(sprintAction);
    }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            // Calculate how fast we should be moving using cached input
            ...
            if (enableSprint && cachedSprintPressed && sprintRemaining > 0f && !isSprintCooldown)
            {
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("version 2.0.0 - New Input System", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        EditorGUILayout.Space();
        ...
        EditorGUILayout.PropertyField(SerFPC.FindProperty("moveAction"), new GUIContent("Move Action", "InputActionReference for player movement (Vector2)."));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("lookAction"), new GUIContent("Look Action", "InputActionReference for camera look (Vector2 pointer delta)."));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("sprintAction"), new GUIContent("Sprint Action", "InputActionReference for sprinting (Button)."));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("jumpAction"), new GUIContent("Jump Action", "InputActionReference for jumping (Button)."));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("crouchAction"), new GUIContent("Crouch Action", "InputActionReference for crouching (Button)."));
        EditorGUILayout.PropertyField(SerFPC.FindProperty("zoomAction"), new GUIContent("Zoom Action", "InputActionReference for zoom toggle (Button). Optional."));
